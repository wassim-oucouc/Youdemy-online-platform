...
class Cours
{
   ...
   {
    $query = 'SELECT course.title,course.ID,course.thumbnail,course.description,course.price,course.created_at,course.document,course.Status ,users.Prenom,users.Nom,users.Photo,categorie.Nom AS categoriename from course INNER JOIN categorie ON course.categorie_id = categorie.ID  INNER join users ON course.Teacher_id = users.ID WHERE users.Role = "Enseignant"';
   ...
   public function Selectcoursesandcategorieandteacher($id)
   {
    $query = "SELECT course.title,users.nom,users.prenom,categorie.nom,course.thumbnail,course.document,course.description,users.Photo from users inner join inscription on users.id = inscription.student_id inner join course on course.ID = inscription.course_id inner join categorie on categorie.id = course.categorie_id where  users.id = :id ";
    $ex = $this->connection->connection()->prepare($query);
    $ex->bindvalue(':id',$id);
    $ex->execute();
    return $ex->fetchAll();
   }
